...
\section{Unity}
As seen in Figure~\ref{image:SystemArch} Unity is the central component to this application. It is the part that the user interacts with and the part where all the data flows in and out of. Having said that, in this section we will discuss the more graphical side of the implementation and how we did that.
...
Once these traits have been generated all that is left to be done is to instantiate them in the virtual 3D environment. The solution to this can be seen in the below snippet.
...
Due to us not being training in 3D modeling and animation here are some links to the models we used and altered:
...
The listing~\ref{lst:aiml1} is a simplified version of a rude bot with no ticket. Bare in mind an AIML file has been constructed for each persona that being, polite, neutral and rude also another three AIML for those personas if they do not have a ticket. Totaling to six AIML files which all can be found on the Github repository.
